...
void LETO_BLDC_Motor::TitanWrite(uint8_t command, int n_uint8_ts) {
  ...
  //  Serial.printf("W CMD: 0x%02X, Add: %d, \tData 0: %d,\tData 1: %d,\tData 2: %d,\tData 3: %d\n",
  //  command, this->I2C_addr,tx_data[0],tx_data[1],tx_data[2],tx_data[3] );
}
...
uint8_t LETO_BLDC_Motor::getContinuous() {
  ...
  // Serial.printf("%c Motor Continuous: %d, %d\r\n",name[0],rx_data[0],rx_data[1]);
  return rx_data[1];
}
...
void LETO_BLDC_Motor::setFirstEndstop(uint16_t _firstEndStop){
  tx_data[0] = (uint8_t)(_firstEndStop >> 8 & 0x00ff);
  tx_data[1] = (uint8_t)(_firstEndStop & 0x00ff);
...
uint16_t LETO_BLDC_Motor::getFirstEndstop(){
  ...
  uint16_t tempFirstStopData = rx_data[0]<<8 + rx_data[0];
