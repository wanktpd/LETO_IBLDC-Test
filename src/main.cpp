...
void setup() {
  ...
  Serial.print("Resetting H motor...\r\n");
  ...
  while (!finished_homing) {
    TitanRead(dev_V_addr, 0x02, rx_data, 1);
    if (rx_data[0] == 0x01) {
      Serial.print("V Homing complete \r\n");
      finished_homing = true;
    } else if (rx_data[0] == 0x02) {
      Serial.print("V Homing failed!\r\n");
      return;
    }
    delay(100);
  }
  ...
  // wait for homing/calibration to complete
  finished_homing = false;
  while (!finished_homing) {
    TitanRead(dev_H_addr, 0x02, rx_data, 1);
    if (rx_data[0] == 0x01) {
      Serial.print("H Homing complete \r\n");
      finished_homing = true;
    } else if (rx_data[0] == 0x02) {
      Serial.print("H Homing failed!\r\n");
      return;
    }
    delay(100);
  }
  ...
  //   tx_data[1] = 0x00;
  //   TitanWrite(dev_H_addr, 0x07, tx_data, 2);
  //   // TitanWrite(dev_V_addr, 0x07, tx_data, 2);
